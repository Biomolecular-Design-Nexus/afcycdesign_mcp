...
GPU Usage:
    python scripts/design_cyclic_sequence.py --input backbone.pdb --chain A --output designed.pdb --gpu 0
    python scripts/design_cyclic_sequence.py --input backbone.pdb --chain A --output designed.pdb --gpu 1 --gpu_mem_fraction 0.8
...
def run_design_cyclic_sequence(
    ...
    **kwargs
    ...
    if positions is not None:
        ...
        # Calculate which positions to fix (not design)
        all_pos = set(range(af_model._len))
        fixed_pos = sorted(all_pos - set(positions))

        # Restart and set fix_pos directly in opt dict
        # Must be numpy array for JAX indexing compatibility
        af_model.restart()
        if fixed_pos:
            af_model.opt["fix_pos"] = np.array(fixed_pos)
    else:
